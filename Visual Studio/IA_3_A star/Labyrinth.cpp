...
{
	if (!openList.empty()) {
		//Retirer de l'open-list un n�ud N minimisant F=G+H
		Node* N = nullptr;
		...
		if (this->isAtExitPoint(N)) {
			...
			for (Node* V : neighboursOfN) {
				...
				if (!found) {
					...
					//Sinon ajouter V dans l'open-list (i.e noeud � traiter)
					if (!found) {
...
{
	//Afficher le chemin en partant de N et en utilisant la propri�t� P�: n�ud pr�c�dsent
